...
- `Voice mandate`：为每次语音授权生成可验证的 mandate hash，绑定 voice hash、intent hash、动作范围和过期时间。
- `Policy decision record`：在确认前检查金额上限、代币白名单、可信收款人和隐私策略，确认不能绕过策略。
- `EIP-712 typed data`：把 voice mandate 导出成钱包可读、可签名的 typed structured data。
- `Proof registry contract`：提供 `VoiceSessionProofRegistry.sol`，后续可把 mandate/proof hash 写到 Pharos。
...
- `AgentSkill/MCP tools`：提供 7 个工具 schema，方便 Agent 编排调用。
...
| `prepared_action.mandate` | 语音授权凭证，包含 mandate hash、scope、过期时间和隐私声明 |
| `prepared_action.policy_decision` | 金额、代币、收款人、隐私检查结果 |
| `prepared_action.challenge` | 用户需要读回的挑战短语，绑定 action id 和 mandate hash |
| `prepared_action.audit_timeline` | 从语音事件到策略、确认、提交的审计链路 |
| `prepared_action.eip712` | 钱包可读的 EIP-712 `VoiceMandate` typed data 和 typed data hash |
| `prepared_action.registry_call` | `VoiceSessionProofRegistry.recordVoiceMandate` 的合约调用预览 |
...
- `demo/pharos_payment_policy_blocked.jsonl`：用户确认了 0.50 PHRS 高额付款，但因超出限额且收款人不可信，策略层仍然阻断。
- `demo/pharos_session_proof_confirmed.jsonl`：用户要求把语音会话证明上链，确认后生成 proof payload。

## 合约草案

`contracts/VoiceSessionProofRegistry.sol` 提供最小 proof registry：

- 只保存 hash、动作、subject、submitter、时间戳和签名字节。
- 不保存原始音频、私钥或完整语音内容。
- 防止同一个 `mandateHash` 重复记录。
- 可作为 Phase 2 Agent Arena 的 Pharos 链上部署起点。
