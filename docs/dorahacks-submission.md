...
- On-chain voice session proof: generates `voice_hash`, `intent_hash`, `action_id`, and proof payload.
- MCP/AgentSkill interface: exposes `process_voice_events`, `prepare_onchain_action`, `confirm_action`, `submit_transaction`, and `write_session_proof`.
